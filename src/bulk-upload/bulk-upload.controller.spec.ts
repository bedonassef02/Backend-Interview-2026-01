...
describe('BulkUploadController', () => {
            ...
            providers: [
                {
                    ...
                    useValue: {
                        ...
                        resetDatabase: jest.fn().mockResolvedValue({ message: 'Database has been reset successfully' }),
                    },
                },
            ],
        }).compile();
    ...
    describe('uploadCSV()', () => {
        ...
        it('should throw BadRequestException when file is undefined', async () => {
            await expect(
                controller.uploadCSV(undefined as any),
        ...
        it('should propagate errors from bulkUploadService.processCSV', async () => {
            ...
            const mockFile = { buffer: Buffer.from(''), originalname: 'bad.csv' } as Express.Multer.File;
    ...
    describe('getRecords()', () => {
        it('should return { success: true, count, records }', async () => {
            const mockRecords = [
                { id: '1', data: { name: 'Alice' }, status: 'processed' as const, createdAt: '' },
                { id: '2', data: { name: 'Bob' }, status: 'processed' as const, createdAt: '' },
            ];
    ...
    describe('resetDatabase()', () => {
        ...
        it('should return the message from the service', async () => {
            ...
            expect(result).toEqual({ message: 'Database has been reset successfully' });
        });
    });
});
